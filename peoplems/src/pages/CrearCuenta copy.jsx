...
export function CrearCuenta({ idCuentaSeleccionada, setIdCuentaSeleccionada }) {
  ...
  function inicializarDatos(datosLeidos = {}, idOportunidad = 0) {
    ...
    const datos = {
      ...
      statusCuenta: 2, //Pendiente de activaci´pn
    };
  ...
  useEffect(() => {
    ...
    const obtenerDatosCuenta = async () => {
      ...
      /*const resDatosCuenta = fetch(cadCuenta, {
  ...
  const registrarCuenta = useCallback(async () => {
    ...
    if (!valido) {
      ...
      const resultadoGrabacion = await grabarRegistro(datos, "cuenta");
  ...
  return (
    <Box sx={{ width: 1 }}>
      ...
      <ListaOportunidades idCuentaSeleccionada={idCuentaSeleccionada} />
